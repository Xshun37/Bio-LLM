...
def _load_gs_review_data():
    """Load GS_50 reference data for the gs_review page."""
    data_dir = os.path.join(PROJECT_ROOT, "data")
    gs_path = os.path.join(data_dir, "GS_50.tsv")
    trrust_path = os.path.join(data_dir, "trrust_rawdata.human.tsv")
    alias_path = os.path.join(data_dir, "gene_alias_index.json")
    abstracts_path = os.path.join(data_dir, "gs50_abstracts.json")
...
def create_app():
    ...
    ASSAY_OPTIONS = [
        ...
        {'tag':'CAT_assay','en':'Chloramphenicol Acetyltransferase Assay','cn':'CAT报告基因检测','category':'Expression/Reporter'},
        ...
        ,{'tag':'ELISA','en':'Enzyme-Linked Immunosorbent Assay','cn':'酶联免疫吸附测定','category':'Protein/Interaction'}
    ]
