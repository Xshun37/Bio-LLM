...
def analyze_tf_interaction(abstract_text, model_name=DEFAULT_MODEL, temperature=0, debug=False):
    round1_user = (
        ...
        "    SAME gene (e.g., activates isoform A, represses isoform B), report it as\n"
        "    ONE relationship. Use the net/dominant effect as the direction, or use\n"
        "    'Regulation' if effects are balanced. Mention the isoform details in the\n"
        "    evidence field. Do NOT output multiple (TF,Target) pairs for isoforms.\n\n"
        "    AUTO-REGULATION RULE: A TF binding to the promoter of Gene X means\n"
        "    TF → X (NOT TF → TF). Only report TF → TF if the abstract explicitly\n"
        "    states that the TF regulates its OWN expression. Binding sites for a TF\n"
        "    on a target's promoter do NOT constitute auto-regulation.\n\n"
