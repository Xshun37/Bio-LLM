#!/usr/bin/env python3
"""Convert gs_review final.tsv into annotation_server standard output.

The output schema matches /api/export_csv in annotation_server/app.py.
The conversion is intentionally offline: ENSG and UniProt values come from
local curated/HGNC files under the Bio-LLM project.
"""
...
DEFAULT_INPUT = Path("gs_review/final.tsv")
DEFAULT_OUTPUT = Path("annotation_server/gs50_standardized.csv")
...
def load_ensg_to_symbol(root: Path) -> dict[str, str]:
    path = root / "data" / "curated" / "gene_ensg_map.json"
...
def load_tf_uniprot(root: Path) -> dict[str, str]:
    path = root / "data" / "raw" / "hgnc_complete_set.txt"
