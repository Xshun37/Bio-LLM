...
from bio_llm.result_utils import load_json, normalize_results_map, relation_list

# Excel (openpyxl) 禁止的字符：\x00-\x08, \x0B, \x0C, \x0E-\x1F
_ILLEGAL_EXCEL_CHARS_RE = re.compile(r'[\x00-\x08\x0b\x0c\x0e-\x1f]')


def _sanitize(s: str) -> str:
    """移除 openpyxl 不接受的非法字符"""
...
def main():
    ...
    data = normalize_results_map(load_json(args.input))

    entries = []
    outputs = []

    for pmid, entry in data.items():
        result = relation_list(entry)
        round1 = _sanitize(entry.get("round1_analysis", "") if isinstance(entry, dict) else "")
        round2 = _sanitize(entry.get("round2_clean", "") if isinstance(entry, dict) else "")
        ...
        if not result:
            ...
            for item in result:
                ...
                entries.append({
                    "PMID": pmid,
                    "TF": _sanitize(item.get("TF", item.get("tf", ""))),
                    "Target": _sanitize(item.get("Target", item.get("target", ""))),
                    "Assay": _sanitize(item.get("assay", item.get("Assay", ""))),
                    "CellLine": _sanitize(item.get("cellLine", item.get("CellLine", item.get("cellline", "")))),
                })
